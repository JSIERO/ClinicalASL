...
for i = 1:SUBJECT.NPLDS
    ...
    SaveDataNII(squeeze(SUBJECT.(prefix).M0ASL_allPLD(:,:,:,:,i,1)), [SUBJECT.ASLdir prefix '_PLD0' num2str(i) '_label1.nii.gz'], SUBJECT.dummyfilenameSaveNII, 1, [], SUBJECT.TR);
    SaveDataNII(squeeze(SUBJECT.(prefix).M0ASL_allPLD(:,:,:,:,i,2)), [SUBJECT.ASLdir prefix '_PLD0' num2str(i) '_label2.nii.gz'], SUBJECT.dummyfilenameSaveNII, 1, [], SUBJECT.TR);
end
...
for i = 1:SUBJECT.NPLDS   
    ...
    SaveDataNII(squeeze(SUBJECT.(prefix).ASL_label1label2_allPLD(:,:,:,:,i)), [SUBJECT.ASLdir prefix '_PLD0' num2str(i) '_label1label2.nii.gz'], SUBJECT.dummyfilenameSaveNII, 1,[], SUBJECT.TR)  % save M0 image , only first PLDs
end
...
SaveDataNII(reshape(SUBJECT.(prefix).ASL_label1label2_allPLD(:,:,:,:,1:2), DIMS(1),DIMS(2), DIMS(3), 2*SUBJECT.NREPEATS*2), [SUBJECT.ASLdir prefix '_1to2PLD_label1label2.nii.gz'], SUBJECT.dummyfilenameSaveNII, 1,[], SUBJECT.TR)  % save interleaved control label and for all PLDs as 4th dimension
...
SaveDataNII(SUBJECT.(prefix).M0_allPLD, [SUBJECT.ASLdir prefix '_M0_allPLD.nii.gz'], SUBJECT.dummyfilenameSaveNII, 1,[], SUBJECT.TR)  % save M0 image all PLDs
disp('Saving M0 image')
SaveDataNII(SUBJECT.(prefix).M0, [SUBJECT.ASLdir prefix '_M0.nii.gz'], SUBJECT.dummyfilenameSaveNII, 1,[], SUBJECT.TR)  % save M0 image , only first PLDs
