...
if isunix  % unix
    ...
    system(['dcm2niix -w 0 -r y -f %p_%s ' DICOMinputdir '> ' DICOMinputdir 'dcm2niix_rename.log 2>&1']);
    ...
    switch nargin
        case 2
            system(['dcm2niix -w 1 -z y -b y -f %p_%s  -o ' NIFTIoutputdir ' ' DICOMinputdir '> ' DICOMinputdir 'dcm2niix.log 2>&1']);
        case 3
            if strcmp(vTR_only,'vTR_only')
                ...
                if ~isempty(files_vTRASL_DCM)
                    ...
                    for i=1:length(files_vTRASL_DCM)
                        system(['dcm2niix -w 2 -z y -b y -f %p_%s -s y -o ' NIFTIoutputdir ' ' fullfile(DICOMinputdir, files_vTRASL_DCM(i,1).name) '> ' DICOMinputdir 'dcm2niix.log 2>&1']);
                    end
                end
                ...
                if ~isempty(files_M0_DCM)
                    ...
                    for i=1:length(files_M0_DCM)
                        system(['dcm2niix -w 2 -z y -b y -f %p_%s -s y -o ' NIFTIoutputdir ' ' DICOMinputdir '\' files_M0_DCM(i,1).name '> ' DICOMinputdir 'dcm2niix.log 2>&1']);
                    end
                end
            end
    end
