...
Description:
    This script implements the main pipeline for processing variable-TR ASL data, including DICOM to NIfTI conversion,
    parameter extraction, , data preparation, brain extraction, motion correction,
...
def prepare_input_output_paths(subject):
    ...
    for context in subject['ASL_CONTEXT']:
        ...
        subject[context]['QASL_CBF_path'] = os.path.join(subject['ASLdir'], f'{context}_QASL_allPLD/output/native/calib_voxelwise/perfusion.nii.gz')
        subject[context]['QASL_AAT_path'] = os.path.join(subject['ASLdir'], f'{context}_QASL_allPLD/output/native/arrival.nii.gz')
...
def get_latest_source_data(subject, context_study_tag, context_tag):
    ...
    #     context_study_tag (str): Study-specific context tag for selecting appropriate files, e.g. 'preACZ', 'postACZ', 
    ...
    context_study_tag_ASL = context_study_tag[0] # e.g. 'preACZ'
    context_study_tag_M0 = context_study_tag[1] # e.g. 'preACZ_M0'

    # Patterns to search for in filenames, default: SeriesDescription patterns matched are ["*SOURCE*vTR*","*SOURCE*M0*"]
    ...
    if subject['is_singleframe']:
        ...
        for fname in os.listdir(dicomdir):
            if context_study_tag_ASL not in fname or not fnmatch.fnmatch(fname.upper(), series_patterns[0].upper()):
                continue
        ...
        if not series_files:
            raise FileNotFoundError(f"No matching DICOM files found for context '{context_study_tag_ASL}' in {dicomdir}")
        ...
        dicom_files = sorted([
            ...
            if fnmatch.fnmatch(fname.upper(), series_patterns[0].upper())  # find matched to first entry patterns, default: '*SOURCE*vTR
            and context_study_tag_ASL in fname
            and fname.endswith('2')
            ])
        if len(dicom_files) > 1:
            warnings.warn(f'Multiple SOURCE ASL DICOM entries found for "{context_study_tag_ASL}". Using latest.')
        if not dicom_files:
            raise FileNotFoundError(f'No SOURCE ASL DICOM entry found for context "{context_study_tag_ASL}" in {dicomdir}')
        dicom_path = os.path.join(dicomdir, dicom_files[-1])

    # Filter NIfTIs
    nifti_files = sorted([
        fname for fname in os.listdir(niftidir)
        if fnmatch.fnmatch(fname.upper(), series_patterns[0].upper()) # find matched to first entry in series_patterns, default: '*SOURCE*vTR
        and context_study_tag_ASL in fname
        and fname.endswith('2.nii.gz')
    ])

    if len(nifti_files) > 1:
        warnings.warn(f'Multiple SOURCE ASL NIFTI files found for "{context_study_tag_ASL}". Using latest.')
    if not nifti_files:
        raise FileNotFoundError(f'No SOURCE ASL NIFTI files found for context "{context_study_tag_ASL}" in {niftidir}')
    ...
    if subject['ASL scan'] == 'multi-delay variable-TR':
        # Filter M0 NIfTIs
        nifti_m0_files = sorted([
        fname for fname in os.listdir(niftidir)
        if fnmatch.fnmatch(fname.upper(), series_patterns[1].upper()) # find matched to second entry in series_patterns, default: '*SOURCE*M0*
        and context_study_tag_M0 in fname
        and fname.endswith('2.nii.gz')
    ])
        nifti_m0_path = os.path.join(niftidir, nifti_m0_files[-1])
        context_data['sourceNIFTI_M0_path'] = nifti_m0_path
...
def mri_diamox_umcu_clinicalasl_cvr(inputdir, outputdir, workingdir, ANALYSIS_PARAMETERS):
    ...
    for i, context in enumerate(subject['ASL_CONTEXT']):
        context_study_tag_ASL = subject['context_study_tags'][i] # e.g 'preACZ' and 'postACZ'
        context_study_tag_M0 = subject['context_study_tags'][i+2] # e.g 'preACZ_M0' and 'postACZ_M0'
        context_study_tag = [context_study_tag_ASL, context_study_tag_M0]
    ...
    ###### Step 10: ASL Quantification analysis
        ...
        # all PLD for CBF and AAT (arterial arrival time map)
        ...
        #                 os.path.join(subject['ASLdir'], f'{context}_QASL_allPLD'),       # output folder name QASL
        #                 context_data['PLDS'], 
        #                 context_data['tau'],
