...
data_T1fit_brain = -1/data_R1fit.*MASK*1e3; % convert decay rate constant to the decay time constant (T1 weighted) and impose mask to include brain-only voxels
data_T1fit_brain(isnan(data_T1fit_brain)) = 0;

data_T1fit_brain_mask_high = data_T1fit_brain<=300; % mask to exclude too high values after fit 
data_T1fit_brain_mask_low = data_T1fit_brain>0; % mask to exclude negative values after fit (e.g. CSF)

T1fromM0 = data_T1fit_brain.*data_T1fit_brain_mask_high.*data_T1fit_brain_mask_low;
