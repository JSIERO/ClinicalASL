...
if new_ext==1
    ...
    if ~exist(fn)
        if exist([fn '.gz'])
            %disp('.nii not found, but .nii.gz found, using this...')
            eval(['!gzip -d -f ' fn])
            fn = sprintf('%s.nii',fileprefix);
        elseif ~exist([fn '.gz'])
            msg = sprintf('Cannot find file "%s.nii" or "%s.nii.gz"', fileprefix);
            error(msg);
        end
    end
elseif new_ext==2
    fn = sprintf('%s.nii.gz',fileprefix);
    if ~exist(fn)
        msg = sprintf('Cannot find file "%s.nii.gz".', fileprefix);
        error(msg);
    end
    eval(['!gzip -d -f ' fn])
    fn = sprintf('%s.nii',fileprefix);
